...
def ingest_directory(directory_path: str, output_path: Optional[str] = None, 
                    supported_extensions: List[str] = None) -> str:
...
def validate_ingestion_output(content: str) -> Tuple[bool, List[str]]:
    ...
    lines = content.split('\n')
    has_content_after_header = any(line.strip() and not line.startswith('# [ARTICLE]') 
                                  and not line.startswith('http') for line in lines)
